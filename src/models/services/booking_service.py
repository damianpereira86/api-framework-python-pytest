...
class BookingService(ServiceBase):
    ...
    def get_booking_ids(
        ...
        return self.request(
            Method.Get, self.url,
            config=config,
            response_model=List[BookingIdResponse]
        )
    ...
    def get_booking(
        ...
        config = config or self.default_config
        return self.request(
            Method.Get, f"{self.url}/{booking_id}",
            config=config,
    ...
    def update_booking(
        ...
        config = config or self.default_config
        return self.request(
            Method.Put, f"{self.url}/{booking_id}",
            booking,
            config,
            BookingDetails
        )
    ...
    def partial_update_booking(
        ...
        config = config or self.default_config
        return self.request(
            Method.Patch, f"{self.url}/{booking_id}",
            booking,
            config,
            BookingDetails
        )
    ...
    def delete_booking(
        ...
        config = config or self.default_config
        return self.request(
            Method.Delete, f"{self.url}/{booking_id}",
            config=config,
            response_model=BookingDetails
        )
